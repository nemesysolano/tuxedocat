...
namespace trading::engine::backtest {
    typedef std::function<expected<unique_ptr<DataHandler>, TuxedoError>(Queue<unique_ptr<Event>> &, const string & , vector<string> &)> datahandler_factory;
    ...
    */
    class Backtest {
        private:
            ...
            ):  csv_dir_(csv_dir), symbol_list_(symbol_list), initial_capital_(initial_capital), heartbeat_(heartbeat), start_date_(start_date), data_handler_(std::move(data_handler)), portfolio_(std::move(portfolio)), execution_handler_(std::move(execution_handler)), strategy_(std::move(strategy)),
