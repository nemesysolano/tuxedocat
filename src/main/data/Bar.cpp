...
using namespace std;
namespace data {
    optional<indexed_result> Bar::nearest_higher_high(const span<Bar> & series){
        if (series.size() < 2) {
            return {};
        }
    ...
    optional<indexed_result> Bar::nearest_lower_low(const span<Bar> & series){
        if (series.size() < 2) {
            return {};
        }
    ...
    double Bar::time_dependent_variance(double x_max, double x_min){
    ...
    std::optional<indexed_result> Bar::time_dependent_variance(const span<Bar> & series){
        auto higher_high_result = Bar::nearest_higher_high(series);
        if(!higher_high_result.has_value()) {
            return {};
        }

        auto lower_low_result = Bar::nearest_lower_low(series);
        ...
        const double variance = Bar::time_dependent_variance(x_max, x_min);
        return pair<size_t, double>(distance, variance);
    }

    optional<double> Bar::inverse_variance_weight(const span<Bar> & series){ // $\hat w(t) = \frac{w(t)}{\displaystyle\sum_{i=0}^{k-1} w(t-i)}$
        if (series.size() < 2) {
            return {};
        }
        ...
        for (size_t offset = 0; offset < series.size(); ++offset) {
            ...
            const auto variance_result = Bar::time_dependent_variance(prefix);
    ...
    optional<double> Bar::scaled_price(const span<Bar> & series) {
        if (series.empty()) {
            return {};
        }

        const auto weight_result = Bar::inverse_variance_weight(series);
